...
5. Put `Status: awaiting approval` directly below that Delivery Plan heading. Change only the selected plan to exact `Status: approved` after explicit human approval; document frontmatter, dated approval text, and another plan's approval do not authorize it.
...
- **Ground it (and catch a broken Map).** Run `codument map check --plan docs/features/<name>.md --json`. Its `grounding` field is the adversary's oracle — the committed invariants, test pointers, dependency edges, and risk tags of every feature the Map routes to, deterministic and identical on every host — so the adversary attacks a real contract instead of hallucinating one. Read `hasMap` and `malformedMap` first:
  ...
  - `hasMap: false` and `malformedMap: false` — skip this map-based pass and disclose it in one line ("no Feature Map — no independent plan pass ran"). Existing scoped files may still have documented invariants; this route does not review them independently. Do not describe the missing Map as an absence of contracts or a clean review.
