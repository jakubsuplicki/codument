...
`;
  ...
  it("a generated SFC classifies coarse — the banner rides an HTML comment the script never sees", () => {
    assert.equal(classifySfcFile(V, "<!-- @generated by tooling -->\n" + BASE).mode, "coarse");
  });

  it("a </script> substring inside a comment or string never splits the block (the Svelte reclassification hole)", () => {
    const svelte = [
      "<script>",
      "// see docs at https://example.com/</script>-tag",
      'const CLOSER = "</script>";',
      "export function foo() {",
      "  return CLOSER;",
      "}",
    ...
    const as = anchors(svelte, "lib/C.svelte");
    assert.ok(named(as, "foo"), "the real export must anchor — never swallowed into the template");
    // A contract change on it is a signature move, not template churn.
    const sigEdit = anchors(svelte.replace("foo()", "foo(x, y)"), "lib/C.svelte");
    assert.notEqual(named(sigEdit, "foo")?.signature, named(as, "foo")?.signature);
  });

  it("a quoted '>' in an opening tag never truncates it (Vue 3.3 generic components)", () => {
    const generic = [
    ...
    assert.equal(classifySfcFile(V, generic).mode, "precise");
    assert.ok(named(anchors(generic), "items"));
  });

  it("a </style> inside a CSS string never splits the style block", () => {
    const tricky = BASE.replace(
      ".hero { color: red; }",
      '.hero::after { content: "</style>"; color: red; }',
    );
    const as = anchors(tricky);
    assert.ok(named(as, "style"));
    assert.ok(named(as, "area"), "the script must survive the tricky style content");
  });
});
