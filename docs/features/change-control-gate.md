...
- **A gate that cannot run fails closed, never green.** Outside a git repository — as with an unreachable base — `review --strict` and `--require-review` exit nonzero with a "gate could not run" diagnostic; bare `review` stays informational. The `--json` contract is discriminated so no consumer reads a missing verdict as a pass: `gate: "ok"` carries the verdict, `gate: "unavailable"` carries a reason, and a null verdict is never emitted. *(test: review.test.ts "review in a non-git directory (fail closed)")*
