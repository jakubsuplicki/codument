...
The CLI is the user-facing entry point for codument. It uses Commander to expose the delivery-workflow lifecycle commands (`init`, `scan`, `doctor`, `review`, `report`, `watch`, `demo`, `migrate-registry`, `adopt`, `update`) plus the `benchmark` proof command family and the `run` signpost.
...
- **`migrate-registry`** — one-shot convert `docs/.registry.json` from the legacy flat shape (or old `mappings`) to v2 with a backup; idempotent on an already-v2 registry (`--dry-run` to preview)
