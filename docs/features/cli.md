...
The CLI is the user-facing entry point for codument. It uses Commander to expose the delivery-workflow lifecycle commands (`init`, `scan`, `doctor`, `review`, `ack`, `report`, `watch`, `demo`, `adopt`, `update`) plus the `benchmark` proof command family and the `run` signpost.
...
- **`doctor`** — reports documentation coverage (ownership, dependency, risk) and registry lint warnings (including doc bloat); `--json` emits the stable report contract. Bloat thresholds are tunable (`--max-doc-lines`, `--max-section-lines`, `--max-completed-log`) as is `--high-fanout`. `--write` persists `.codument/coverage.json` and the SVG badge. `--strict` makes findings exit 1 for CI gating (opt-in). Warning-only by default: without `--strict`, findings never change the exit code, and notes never do.
- **`review`** — reviews the uncommitted git diff against the registry: changed files grouped by owner, stale docs, high-risk touches, out-of-plan changes (when an approved plan is detected), unmapped changes, high-fanout files, and dependent features. `--json` emits the review contract; `--log` appends a review event to `.codument/events.jsonl`; `--strict` exits 1 while the change left a new source unmapped or a mapped doc stale; `--base <ref>` reviews the branch's drift since it diverged from `<ref>` (merge-base..working-tree), not just uncommitted changes, failing closed if the base is unreachable. Reports facts and gaps; it does not certify safety.
- **`ack`** — records a decision that a moved symbol owes no doc change (a pure-internal refactor), so `review` stops flagging it. The agent names the anchor (`<path>::<symbol>` — the exact line `review` prints, or a bare symbol name resolved against the moved set, failing loud on collision) and a `--reason`; the command recomputes the exact `from`→`to` fingerprint itself, so no fingerprint is ever copied. The decision is fingerprint-bound and auto-invalidates the next time the symbol moves. `--list` and `--remove <handle>` make acks visible and reversible; `--base <ref>` matches `review --base`. This is the reachable surface of the agent-judge loop: update the doc when a contract changed, ack when it did not.
