...
- **Clean ≠ empty tree** — a `✓ CLEAN` verdict means nothing codument *governs* (source or docs) changed, not that the working tree is empty. When only config/asset files change (e.g. `app.json`, an image), the gloss reads `"N files changed · not source or docs"` — never "working tree clean". `ChangeState` keeps these as `otherChanged`, the third bucket of the `sources ∪ docs ∪ other` partition, so the verdict can't silently imply an empty tree while real files sit uncommitted. (Caught dogfooding the verdict frame against a real repo, 2026-06-19.)
