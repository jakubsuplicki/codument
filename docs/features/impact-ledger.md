...
**Soak line (drift calibration, info-only).** A `caught` event also carries a `DriftTally` — the per-symbol drift outcome for that snapshot: how many owned anchors moved, how those moves split into **contract (signature) vs body-only churn** (a signature move is never ackable and always owes a doc update; a body-only move is reported and never gates, so it owes neither a doc line nor a signature — see [[change-control-gate]]), how many were **resolved by a doc update** (the owning doc changed in the diff — the same verdict-derived resolution the gate uses), how many were cleared by an **acknowledgment** — split into per-symbol refactor acks and file-grain additive acks (`ack <path>`), both of which owe no doc change — and a separate co-movement breakdown kept only as info-only telemetry for calibrating co-movement itself. Summed across snapshots by `summarizeImpact` into a `DriftLedger`, this is the **soak signal** for the freshness gate: friction is the share of *resolved* fires that owed no doc change (either ack kind) rather than a real doc update. Counting a file-grain ack on the ack side, never as a doc update, is what keeps friction honest (see [[change-control-gate]] / ADR 012). The contract/body split lets the info-only → blocking flip read unavoidable contract work apart from the implementation churn the gate reports without charging for it, rather than one blended fire count. It calibrates whether the deterministic stale-doc verdict is quiet enough to become a required CI check (the info-only → blocking flip). Co-movement is never a resolution input (see [[change-control-gate]] / ADR 010); the soak never blends into the provable/reported headline; and it is always labeled info-only.

**Ack audit events (change-control, not part of the ledger).** `review-events.ts` also defines `emitAck`/`emitAckRemove`, which append identity-bearing `ack` / `ack-remove` records to the same `events.jsonl` when the change-control gate records or retracts an acknowledgment — carrying the anchor, its `from`→`to` fingerprint, the reason, the signer, self-vs-independent, and — where the exemption covered a whole governed tree — how many files it covered, so a wide vouch cannot read in the log as a vouch over one path. They are the durable audit trail that makes an agent's self-resolved drift auditable from the log alone (see [[change-control-gate]]); they share the log and this module, never the Caught tally.
...
- **A command that changed state says what it recorded, and says whose claim it is.** `emit review` wrote in silence, which put the one command the review loop runs most on the wrong side of the same question the ledger is built around: a caller with no output cannot tell a recorded finding from a mistyped flag, and in the field it was run many times into nothing. It echoes the record now — the record itself, returned by the writer rather than rebuilt beside it, because an echo assembled from the inputs is a second definition of what was written and the surface that reports a thing must not be able to disagree with the thing. What the echo must not be is a success mark. This is the self-reported line: an agent's claim about its own work, which codument stores, does not check, and deliberately discounts in the headline. A tick would read as codument vouching for it — one more confident green in a loop whose greens were the problem — so the line is informational, and carries the words *self-reported* itself. A refusal echoes nothing, because nothing was recorded. *(tests: `emit-producer.test.ts` "emit review says what it recorded" — every field of the appended record present in the line, the self-reported marking with no success mark, absent attribution omitted rather than rendered empty, and a malformed flag echoing nothing with nothing in the log)*
...
- **The ledger is queryable, not just rendered.** Beyond the `watch` frame and the shareable HTML page, the whole tally rides the versioned, timestamp-free `report --json` contract (alongside the acks card — see [[change-control-gate]]), so CI or a script can read the Caught numbers instead of scraping a rendered surface. It is the same pure `summarizeImpact` tally the human surfaces render, so the three cannot disagree. *(test: report.test.ts "report --json (machine surface)" — the ledger rides the versioned envelope, byte-identical across runs)*

## Non-goals

- No new top-level command. The ledger lives in `watch`; the shareable copy lives in `report` (its machine form in `report --json`). (`emit review`, `review --log`, and `report --json` are subcommands and flags on existing commands, not new commands.)
