...
- Scope and Feature Map complement each other. Existing source, direct documentation, and registered workflow instructions have reviewable owners even when a plan adds no file. Missing owners, dependencies, or documents remain named limitations; they never justify a claim of clean review.

## Invariants & boundaries

- Scoped sources, direct docs and registered instructions use the shared owner resolver; missing docs and unknown dependencies are explicit omissions. *(test: `plan-grounding.test.ts`)*
...
- **Step SHAPE is an objection class of its own, and the plan's own text is its citation.** A step is the unit `work-step` → `review-work` → `commit-work` each hold exactly once, so a step that repeats one operation over a list of artifacts breaks all three together: implementation outruns the session, review over dozens of parallel outputs is not review, and the commit is a monolith. It is uniquely the adversary's to catch — the grill resolves assumptions and this is a property of the checklist, and it is visible at exactly one moment, before approval, when the count is written down and no agent has spawned. It also breaks the citation pattern deliberately: a step sentence carrying a count or a plural ("all twelve locales") IS the grounding, so this objection needs no invariant behind it, which is why it must be named as its own class rather than left to the general rule that would drop it as ungrounded. The objection names both restructures and, as with every other, the user adjudicates. *(enforced by the `adversarial-planner` mandate and the sizing rule in `plan-with-docs`; no code gate — a deterministic step-size lint would have to guess "too big" from checklist prose, and a gate that guesses erodes the ones that do not)*
...
- Plans without new files receive the same owner grounding through explicit Scope. Map shape diagnostics remain separate from whether contracts were retrieved. *(test: `plan-grounding.test.ts`)*
