...
- [ ] **Phase 2 — TS per-symbol anchors.** Token-stream fingerprint, SCIP-shaped identity (order-independent), same-file transitive closure + file-hash backstop, symbol-grained ownership (derived-first + explicit for shared files, import-graph-seeded), barrel/generated/export-form classification, parse-error fail-loud, per-file `precise|coarse` mode in the facts output. Acceptance: a shared-file single-symbol edit wakes exactly the owning doc; reorder/rename/move/delete fixtures behave as specified.
  - **Slice 2a landed** (`src/lib/ts-adapter.ts` + the per-symbol `anchors()` seam in `fingerprint.ts`): syntactic exported-symbol extraction, a token-stream fingerprint per declaration (`ts.createScanner`/`getTokenText`, body-inclusive), SCIP-descriptor-shaped order-independent identity, and `changedAnchors` (per-symbol two-ref diff). Cascade dissolution, reorder-insensitivity, and token-stream invariance (`0x10`≠`16`, intra-string edits caught) are proven in `tests/ts-adapter.test.ts`.
  - **Slice 2b landed** (`src/lib/ts-adapter.ts`): each export's fingerprint is now transitively closed over the same-file non-exported declarations it references (lexical, no type resolution, conservative over-approximation — a shadowing local only over-wakes, never under-wakes), closing the private-helper blindness while keeping closure per-export (a helper used by `f` not `g` still wakes only `f`). Plus a residual backstop anchor (`path::<module>`, kind `module`) hashing every top-level statement no precise anchor covers — imports, side effects, unreferenced module state — emitted only when residual content exists and excluding closure-covered helpers (no double-count). Proven in `tests/ts-adapter.test.ts` (closure direct/transitive/per-export, backstop for side-effects/imports/dead state, no-residual and no-double-count cases).
  - **Slice 2c landed** (`src/lib/ownership.ts` + an optional `owned_symbols` registry field): `resolveOwner(registry, anchorId)` resolves a symbol's owning feature derived-first — a file in exactly one feature's `primary_sources` owns all its symbols (zero authoring, the common case), ownership from `primary_sources` not `related_sources`; a file shared across several features is split per-symbol by the `owned_symbols` map (`Record<path, descriptor[]>`), returning `owned` / `unowned` / `unassigned` / `ambiguous` so the gate fails loud rather than silently waking all co-owners. Pure + fully tested (`tests/ownership.test.ts`). (Also removed a dead `basename`/`extname` import left in `registry.ts` by the Phase 0 migration removal.)
  - **Slice 2c-ii (harvest) landed** (`src/lib/import-graph.ts`): first-party import harvesting from the same syntactic parse — `resolveSpecifier` (relative `.js`→`.ts` source mapping, `..` normalized, external/bare → null), `harvestImports` (per-binding `local`→`resolved`, type-only included, side-effect excluded), `importedFiles` (deduped sorted edges). Pure, fs-free, fully tested (`tests/import-graph.test.ts`). This is the foundation for both the ownership seeding and the facts/graph import-graph contract.
  - **Slice 2e (gate wiring) landed — the payoff** (`src/lib/change-state.ts`, `src/lib/fingerprint.ts`, `src/commands/review.ts`): `computeChangeState` now takes an optional per-file `anchorChanges` map and resolves doc-staleness PER-SYMBOL — each changed anchor routes through `resolveOwner` to exactly its owning feature, so a one-symbol edit to a shared file wakes only that symbol's doc (the cascade is dissolved). Ownership is `primary_sources` only (related_sources is impact, never staleness); concept umbrellas wake at **file grain** whenever an owned file's content moves; an `unassigned`/`ambiguous` shared symbol wakes every candidate AND surfaces an `ownershipLints` finding (fail-loud, never silently waking all). A changed file with no anchor data (coarse/non-TS, or anchors uncomputable on a fresh repo) falls back to file-grain ownership. `computeChangeState` stays **pure**: `fingerprint.ts` gained `isPreciseFile`, `changedAnchorsAgainstWorktree` (base ref vs the working tree on disk — the head `review`/`watch` actually evaluate), and `gatherAnchorChanges` (best-effort, degrades to file-grain), which `buildReview` calls and threads in (default base `HEAD`; `review --base` passes the resolved merge-base). `byFeature`/`riskTouches`/`dependents` stay the broad primary+related impact view by design. The one genuine multi-feature split in codument's own tree — `src/commands/benchmark.ts` (`commands` + `proof-benchmarks`) — is hand-authored: `proof-benchmarks` owns both its anchors (`createBenchmarkCommand().` + `<module>`) since proof-benchmarks.md carries the command's behavioral narrative; all 14 other shared files derive cleanly. Golden table in `tests/gate-wiring.test.ts` fixes exactly which scenarios flip the verdict (cascade dissolution, concept file-grain, cosmetic-immunity, related-only never stale, unassigned/ambiguous fail-loud, file-grain fallback, docs-changed-without-source) plus an end-to-end temp-repo proof over the real git+disk anchor diff.
  - **Slice 2d (classification + parse-error fail-loud) landed** (`src/lib/ts-adapter.ts`, `src/lib/fingerprint.ts`, `src/lib/change-state.ts`, `src/commands/review.ts`): `classifyTsFile(path, content)` returns a per-file mode — `precise` (≥1 exported symbol → per-symbol gate applies), `coarse` (`.d.ts`, an `@generated`/DO-NOT-EDIT banner, a re-export barrel, `export =`, a namespace, a side-effect-only or comments-only module → gated whole-file), or `unevaluable` (a parse error / conflict markers / syntax newer than the pinned parser → fail loud). `gatherAnchorChanges` now classifies each changed file from its head content and includes ONLY `precise` files in the per-symbol map; `coarse` files are omitted so the change-state falls back to file-grain ownership — which **closes the empty-anchor-set-reads-fresh hole** (a `.ts` file whose real surface the precise extractor can't anchor, e.g. comments-only, no longer slips through fresh) — and `unevaluable` files are omitted (file-grain, never fresh) AND surfaced: `ChangeState.unevaluable` carries them and `review` prints a "could not evaluate (parse error)" section. The per-file mode is the `precise|coarse` signal the Phase 4 facts output will carry. Proven in `tests/ts-adapter.test.ts` (11 classification cases: precise, parse-error, conflict-markers, `.d.ts`, generated banner, re-export barrel, `export =`, namespace, side-effect-only, comments-only, mixed precise+re-export) and `tests/gate-wiring.test.ts` (e2e: a parse error surfaced + gated file-grain; a no-anchorable-content file wakes its owners file-grain). NOTE: outDir-based generated detection (needs tsconfig) and one-hop within-repo barrel resolution are deferred refinements, not gate-correctness gaps — the `<module>` backstop + coarse fallback already gate those files safely; classification makes the handling explicit/honest.
  - **Remaining slices:** apply the harvest to seed `owned_symbols` for shared files (a shared-file symbol that references feature X's exclusively-owned files → X; needs per-symbol reference extraction from `ts-adapter`, a small refactor of the proven anchor path) + `scan` integration + a doctor lint (repo-wide) on shared files with unassigned exported symbols. This is the last Phase-2 slice and is **deferred gold-plating** — codument's own tree has exactly one genuine multi-feature shared file (`benchmark.ts`, already hand-authored), so auto-seeding is a general-case convenience, not dogfood-critical.
- [ ] **Phase 3 — Agent-judge-centric drift resolution + acknowledgments (info-only).** Reshaped 2026-06-27 (see the Co-movement note above): the centerpiece is **the precise finding + the agent reconciling it inline + a recorded acknowledgment**, NOT a deterministic prose-match gate. Deliverables: (1) **precise per-symbol drift trace** — surface the moved owned anchors (symbol id, fingerprint `from`→`to`, change kind) behind each flagged doc so the agent and an acknowledgment can bind to an exact fingerprint; (2) **acknowledgment protocol** — loose, reviewable `.codument/acks/*.json` files scoped to `{anchorId, fromHash, toHash, reason, signer}`, **fingerprint-bound so it auto-invalidates when the anchor moves again** (no ride-forever); a valid ack suppresses its finding (a recorded, attributed decision, never a behavior-verified fact); (3) **co-movement telemetry** — the symbol-scoped prose-moved signal computed and **logged info-only** (soak calibration), never a verdict input; (4) **events** — every catch / auto-fix / surface appends to `.codument/events.jsonl` with the symbol+fingerprint trace, feeding both the soak and the value metrics. **Resolution is agent-default** (autopilot-aligned: agent fixes the doc or writes the ack inline, pauses only on judgment/sensitive); independence (signer ≠ change author) is **opt-in strict mode**. Signature-only-ineligibility is deferred until signature-vs-body anchors are distinguished (currently body-inclusive only).
  - **Primitives landed** (pure, fully tested): the **acknowledgment protocol** (`src/lib/acknowledgment.ts` — `parseAck`/`ackCovers`/`ackFileName`/`readAcks`/`writeAck`/`isIndependent`; fingerprint-bound, auto-invalidating, deterministic loose-file storage) and the **co-movement telemetry primitive** (`src/lib/co-movement.ts` — `classifyComovement` → `co-moved`/`not-referenced`/`prose-unchanged`/`no-doc`, frontmatter/link-URL-stripped, whole-identifier symbol match; info-only, never a verdict input).
  - **Drift wiring landed** (`src/lib/drift.ts`, `src/commands/review.ts`, `src/lib/fingerprint.ts`): `AnchorChange` now carries the `from`→`to` fingerprints; `computeDrift` resolves each moved OWNED anchor to its feature + doc, attaches the co-movement telemetry status, and checks whether a recorded ack covers the exact transition. An **acked move is adjudicated** — dropped from the anchor-change set the deterministic stale-doc verdict sees, so a recorded "refactor, no doc change owed" decision clears the flag (and auto-invalidates on the next move, since the ack binds the `to` fingerprint). `buildReview` reads acks, runs `computeDrift`, feeds the filtered changes to `computeChangeState`, and exposes `ReviewReport.drift`; `review` surfaces the precise per-symbol trace (symbol, change kind, co-movement status, owning doc) info-only and notes the count cleared by acknowledgments. Proven end-to-end in `tests/drift.test.ts` (drift finding + fingerprints; co-moved when the symbol's doc line is reconciled; ack clears the verdict; ack auto-invalidates on re-move).
  - **Soak instrumentation landed** (`src/lib/review-events.ts`, `src/lib/impact-ledger.ts`, `src/commands/review.ts`, `src/commands/watch.ts`): a `caught` snapshot now carries a `DriftTally` (`flagged`/`coMoved`/`proseUnchanged`/`notReferenced`/`acknowledged`); `review --log` computes it from `ReviewReport.drift` at the commit boundary; `summarizeImpact` sums it across snapshots into a `DriftLedger` with `frictionRate = acknowledged / (acknowledged + coMoved)`; `watch` renders a `soak` line under the Caught block (info-only). This is the calibration data that decides the info-only → blocking flip — **it now self-collects in real usage** (no extra instrumentation needed). Validated on the `change-control` fixture project: 3/4 common refactors (whitespace, comments, reorder) produced ZERO fire; the one false-fire (a local-variable rename — the known token-stream limitation) was correctly traced and cleared by an ack; 0 false-negatives (every behavior change fired, including a private-helper edit via the closure); generated files and parse errors handled; the ack auto-invalidated on re-move; a "laundered reconcile" (touch an unrelated doc line) cleared the file-grain verdict but the co-movement telemetry still flagged it — exactly the signal the soak is meant to capture.
- [ ] **Phase 4 — Gate it.** (Soak instrumentation above is the buildable half and has landed; the FLIP below is the soak-data-dependent half.) Co-movement/acknowledgment becomes blocking only after the info-only soak shows acceptable false-fire — **falsifiable in form even though the number is calibrated from live data**: the info-only -> blocking flip is permitted only when a recorded false-fire rate at or below a threshold **written into this doc before flipping** is met over a stated soak window (a minimum flagged-anchor count `N` and repo count `M`); the `rate/N/M` are filled from the Phase 3 `events.jsonl` data, not pre-guessed, so "soak passed" is a check rather than a vibe. CI wiring (required check, graceful fork/read-only degrade, base/head/version/algoStamp echoed). Cleaned `facts --json` / `graph` contract emitted. Surface the **caught / auto-fixed / surfaced** counts (read from `.codument/events.jsonl`) in `review`/`report`/`--json` as the value metrics.
...
- [x] **Step W1 — `codument ack` command (the reachable surface).** New `ack` command in `src/cli.ts` + `src/commands/ack.ts`: `codument ack <path::descriptor> --reason "<text>" [--base <ref>] [--signer <id>]`. The positional arg is the full `anchorId` (what the system actually models); a bare name is also accepted and resolved against the moved-anchor set, **failing loud with the candidate descriptors on collision** (overloads, default exports, same-name term+type). It runs `gatherAnchorChanges` for the file against the base (default `HEAD`), finds the matching moved anchor, and writes the fingerprint-bound ack via the existing `writeAck`. Add `codument ack --list` (stable human handle — the `ackFileName` digest prefix — plus reason/signer/validity) and `codument ack --remove <handle>` (itself an audited event), with a list->remove round-trip test. Verify the `()`/`#`/`default.`/`::` descriptor round-trips through the shell arg unescaped. Unit + CLI tests (full-descriptor write, bare-name resolution, collision fail-loud, not-moved rejection, auto-computed fingerprints, list/remove round-trip, auto-invalidation after a re-move). Register `src/commands/ack.ts` in `docs/.registry.json`.
- [x] **Step W1a — Real audit event + git author resolver (the thing that makes self-ack defensible).** Today `events.jsonl` carries only an aggregate `DriftTally` count (`review-events.ts`) — no identity. Add an append-only ack event `{anchorId, fromHash, toHash, reason, signer, kind: self|independent}` emitted **at ack time** (the data is already on `Acknowledgment`/`DriftFinding`), and an `ack --remove` event. Implement the signer/author resolver in `src/lib/git.ts` (`git config user.name`/`user.email` or `git var GIT_AUTHOR_IDENT`), with the literal `agent` fallback only when git is absent — so a self-ack (signer == change author) is detectable in the log from day one. Unit-test both. This is the durable, identity-bearing trail; the loose ack file is no longer the only record the agent controls.
- [x] **Step W2 — Surface the affordances in `review` as a fork.** `review` (human + `--json`) prints `d.anchorId` (currently only `d.symbol` is printed, `review.ts:256`) and renders each drift finding as an explicit two-arm fork: **"contract changed -> update `<doc>` at intent altitude"** AND **"internal refactor only -> `codument ack <path::descriptor> --reason \"...\"`"** (the exact copy-pasteable line). Add a first-class per-run summary line ("N moves acked as contract-neutral · M docs updated") so an all-ack change is loud, and a section listing acks already applied. Tests in `tests/review.test.ts` for the fork lines, the ack-rate summary, the applied-acks list, and the `--json` fields.
- [x] **Step W3 — Wire the loop (automatic in-flow), all THREE skill copies.** Update `review-work` in **`.agents/skills/`, `.claude/skills/`, and `skills/`** (all three are live roots — verified): after running `codument review`, resolve every drift finding inline with the explicit default bias — **when in doubt, update the doc; ack only when you can name the preserved contract in one clause** — auto-applied without a stop, pausing only on a judgment call or a sensitive change (public interface / security / data loss / ambiguity). Re-run `codument review` to confirm the findings clear. Also add the update-vs-ack two-way-call to the **intent-routing rule in the generated `AGENTS.md`/`CLAUDE.md`**, not just the skill, so a lightweight out-of-loop self-review that moves an owned anchor still resolves rather than committing red/rubbished. No new gate; this rides inside the review that already runs.
- [x] **Step W4 — Altitude rule into the contract + reconcile the contradiction.** Add the example-driven altitude rule (bad-mirror vs good-contract pair) and the per-edit review-lens question to `review-work` and `update-docs` (**all three skill roots**) and the generated `AGENTS.md`/`CLAUDE.md`. **Critically, add `.claude/rules/documentation.md` (and any generated equivalent) to the edit set** — it currently mandates "Set `last_updated` to today" (removed entirely in the Freshness Gate Redesign Phase 0) and "Update any sections that no longer match your changes," a per-file update-on-touch chore that directly **trains the mirror-edit rubbish this rule fights**; strip the `last_updated` mandate and replace update-on-touch with the altitude/ack branch. Reconcile `update-docs/SKILL.md`'s stale `last_updated`/`sources`-array template remnants in the same pass so the agent never reads contradictory guidance at write time. A test asserts the generated contract carries the altitude rule and the ack branch, and carries no `last_updated` mandate.
- [x] **Step W5 — Docs, registry, and a blocking scripted dogfood gate.** Document the `ack` command in `docs/features/cli.md`; update this doc and `docs/concepts/lib.md` for the new command + audit event + review fork; register the new sources; bump `last_reviewed` on touched docs; mark the Phase 3 line reachable. **Blocking acceptance gate (the design has never run as a CLI flow because the command did not exist):** a scripted end-to-end on codument's own tree — make a contract-neutral refactor on a known symbol, run `codument review` and capture the printed command, run that exact command verbatim, re-run `review` and assert the finding is gone, then re-move the symbol and assert the ack auto-invalidated and the finding returns. Dogfood: `review` clean, `doctor --strict` exits 0, `npm run build` + `npm test` green.
