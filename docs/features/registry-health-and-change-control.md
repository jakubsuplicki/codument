...
- [ ] **Phase 1 — Coarse universal hash.** Whole-file token/content hash (BOM/EOL-normalized) for all languages — the universal fallback and the non-TS gate. Kills cosmetic churn + the date-bump game at file grain. The `LanguageAdapter` seam is introduced here.
