...
- `src/lib/claude-feed.ts` — Claude Code session-transcript adapter: discovers the active session log, normalizes per-turn usage + tool activity into events, idempotent tailing (`resolveSessionLog`, `pumpFeed`, `featureForFile`, `normalizeModelId`).
- `src/commands/feed.ts` — `codument feed` CLI: one-shot (`--once`) or continuous tail of the session log into `.codument/events.jsonl`.
- `src/commands/watch.ts` — renders the estimated token-cost block; auto-runs the feed unless `--no-feed` (related).
- `src/lib/events.ts` — the append-only `.codument/events.jsonl` log this rides on (related).
