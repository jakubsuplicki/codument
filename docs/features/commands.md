...
- `scan` classifies known utility directory names as concepts and everything else as features, and discovers files through the analyzer's ONE source spec (extensions and exclusion globs alike — never a local extension list), so it proposes exactly the set the gate will govern: generated/tool directories, `.d.ts` artifacts, and each language's test conventions are skipped. *(tests: `scan.test.ts` "classifies concept directories correctly", "ignores generated and tool directories", "excludes .d.ts files")*
