...
`resolveOwner(registry, anchorId)` answers "which feature does the gate wake when this anchor moves?", **derived-first**: a file in exactly one feature's `primary_sources` owns all its symbols (including the `<module>` backstop) with zero authoring — the common case. Ownership comes from `primary_sources` (owned), never `related_sources` (merely impacted). A file SHARED across several features' `primary_sources` is split per-symbol by an optional `owned_symbols` registry field (`Record<path, descriptor[]>`, the descriptor being an anchor id's `::`-tail): the resolver routes a claimed symbol to its single owner — this is what actually dissolves the `cli.ts` cascade — and returns `unassigned` (a symbol no co-owner claims) or `ambiguous` (two claim it) so the gate fails loud rather than silently waking every co-owner. `splitAnchorId` splits `path::descriptor` on the first `::` (paths never contain it). The resolver is pure; seeding `owned_symbols` from the import graph and wiring the result into the verdict are the next slices.
