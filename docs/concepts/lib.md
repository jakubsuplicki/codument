...
- **Coverage** — scored ratios with an explicit denominator: ownership (in-scope source files with a documented owner), dependency (mature entries declaring `depends_on`; a depended-upon foundation that declares none is a vacuous case, excluded from the denominator rather than scored as a miss), and risk (declared high-risk areas with a durable doc). The headline score is the equal-weight average of the ratios whose denominator is non-zero; a zero-denominator ratio is excluded, never counted as 0% or 100%. (Freshness/drift is no longer a coverage ratio — it is being re-sourced from the unified two-ref freshness gate; see `two-ref.ts` below.)
- **Lint** — typed warnings reported as counts with evidence, never folded into the score: missing/leaked sources, missing docs, high-fanout files, empty `depends_on` on an isolated mature entry (one that nothing depends on — a depended-upon foundation is exempt), unmapped in-scope sources, and bloated docs. Bloat is measured by three independent signals (whole-doc lines, oversized section, completed-log `[x]` accumulation) with conservative CLI-overridable thresholds (`DEFAULT_BLOAT_THRESHOLDS`), calibrated against `fixtures/benchmarks/doc-bloat`.
...
`computeChangeState` is the shared, deterministic diff analyzer behind both `review` (snapshot) and `watch` (live) — so the two can never disagree. It is a pure function of `(registry, changedFiles, optional planScope, optional anchorChanges)`: no git, no clock, fully sorted. From a list of changed paths it derives changed sources grouped by owning feature, unmapped changes, stale docs, docs changed without source, high-fanout changed files, risk touches (changes in a risk-tagged feature), dependents (the blast radius of changed features), out-of-plan changes when a plan scope is supplied, and `ownershipLints`. **Doc-staleness is symbol-grained**: when the caller supplies `anchorChanges` (per-file changed anchors), each changed anchor routes through `resolveOwner` to exactly its owning feature, so a one-symbol edit to a shared file wakes only that symbol's doc — the cascade dissolved. Ownership for staleness is `primary_sources` only (related is impact, not ownership, so it feeds `byFeature`/`riskTouches`/`dependents` but never staleness); concept umbrellas wake at file grain when an owned file's content moves; a shared symbol no co-owner claims (or that two claim) wakes every candidate and is surfaced as an `ownershipLints` finding (fail-loud, never silent). A changed file with no anchor data falls back to file-grain primary ownership; a changed TS file that could not be parsed is echoed into `unevaluable` (gated file-grain, surfaced). The analyzer stays pure — the caller computes anchors + classification (see `fingerprint.ts`) and passes them in.

`git.ts` is the thin git-native data source: `isGitRepo` and `getWorkingTreeChanges` shell out to the already-required `git` CLI with `GIT_OPTIONAL_LOCKS=0` (so polling does not create lock churn that re-triggers the agent), returning sorted repo-relative working-tree changes (deletions excluded). `getGitAuthor` reads the configured identity (`user.name`/`user.email`) so an acknowledgment can record who signed and detect a self-ack (signer == change author), falling back to a generic `agent` outside git rather than fabricating attribution. It is kept separate from `computeChangeState` so the analyzer stays pure and testable without a repo.
...
The managed section also carries the intent router for new chats. It tells agents to enter `grill-with-docs` for rough or ambiguous ideas, move to `plan-with-docs` once scope is settled, start `work-step` only from an approved plan, then require `review-work` and offer `commit-work` as a user-approved gate before the next implementation step. It further encodes the **quality bar** (best-effort, adversarial zoom-out before calling work done; self-resolve, escalate only on genuine ambiguity) and the **documentation-altitude standard** — the layered doc structure plus the two-way call for a moved symbol: update the doc at intent altitude when a contract changed, or `codument ack` a contract-neutral refactor.
