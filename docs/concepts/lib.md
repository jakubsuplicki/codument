...
- Side effects are confined to seams: the analyzers never read the clock or the network; git and event-log access live in `git.ts` and `events.ts`. Git access is bounded (a generous subprocess buffer so a large tree's output is never silently truncated) and the change-listing calls raise rather than return empty on failure, so a broken or oversized git invocation fails the gate loud instead of reading "clean." *(boundary — per-module invariants live in the owning feature docs)*
