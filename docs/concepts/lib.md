...
- `src/lib/exclusion-spec.ts` — the canonical exclusion spec (`DEFAULT_EXCLUSION_SPEC`) and its matchers, a leaf module so the git seam can prune its member walk on the same directory list the analyzers score by, without importing the analyzer.
