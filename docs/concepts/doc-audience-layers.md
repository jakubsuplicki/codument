...
- **A fence is not an exemption from the altitude rule.** A code block reproducing a declaration the doc's own feature owns is the purest mechanism a doc can carry: it is stale the moment that declaration moves, it owes a hand edit every time, and the change-control gate will demand one — so the standard forbidding mechanism and the gate enforcing freshness together compel exactly the rot the standard exists to stop. Illustrations are still welcome; what the rule forbids is reproducing something the reader could read from the source. The prose smells all skip fenced content by construction (rightly, for illustrations), which is why the machine reading of this line is its own smell rather than an extension of theirs. *(the `fenced-mirror` smell in [[registry-health]], info-only in doctor's Notes like its siblings)*
...
- **Compaction on ship is checked, not merely mandated.** A registered doc whose delivery checklist is fully complete is named by the surface that reads every doc, because that is the state the standard is most explicit about and the one nothing was watching: this repository obeyed the rule only because it was done by hand, every time, which is the enforcement gap the tool exists to close. The finding names the doc; it never compacts anything, since deciding what survives into Decisions or an ADR is the judgement compaction *is*. *(the `shipped-scaffolding` finding in [[registry-health]] — a real lint rather than an altitude note, because a completed checklist is a fact about the document, not a heuristic about its prose)*
- **A recorded decision points at something.** The Decisions layer is pointers — the durable why, referenced, never restated — so a layer of bare conclusions records reasoning nobody can re-derive or contest, which is how a wrong recorded decision outlives the attempts to fix what it was wrong about. Judged per layer, not per bullet: a small decision legitimately has no ADR, and a per-bullet rule would only teach authors to decorate every line. Citation is checkable and sufficiency is not, so the check asks only that evidence exists. *(the `unsourced-decision` smell in [[registry-health]], info-only in doctor's Notes like its siblings)*
...
- **The standard has a deterministic reading, shipped info-only.** The altitude lint (see [[registry-health]]) scores every registered doc for five named smells — `symbol-mirror`, `line-anchor`, `path-enumeration`, `fenced-mirror` (the one that reads inside fences rather than skipping them), and `unsourced-decision` (the only one judged per section rather than per line) — with no NLP and no model call. It renders in doctor's Notes channel and never fails `--strict`; promotion of any one id to a warning is a separate decision gated on that id's own false-fire soak, exactly like the change-control gate's info→blocking flip and co-movement's info-only stance ([005](../architecture/decisions/005-co-movement-info-only-telemetry.md)). **Initial soak baseline (this repo, at ship):** `line-anchor` is high-precision — every fire was a real rotting anchor (all fixed in the dogfood pass), and it is scoped to known source/doc extensions so a `host:port` URL never false-fires; `symbol-mirror` over-fires whenever an exported name is a common English word at sentence start — a command name that is also an exported function ("init writes the scaffold"), or a noun-subject like `State`/`Config` ("State transitions must remain atomic") — a class a lexical verb heuristic structurally cannot separate without NLP, so its fires are mostly false and it is the least ready to promote; `path-enumeration` is mixed — it correctly flags a prose section that restates the file list, but also fires on transient delivery-plan / impact sections that legitimately enumerate their scope. `fenced-mirror` ships with no soak history at all and zero fires on this repo, which is the expected opening reading rather than evidence of precision — it demands a declaration site of a symbol the entry owns, so a fence that merely calls one, or shows CLI output, or declares a name from somewhere else, stays silent by construction. `unsourced-decision` opens with no soak either, and one correction is worth recording with it: section scoping had never worked on a CRLF checkout, so every doc was read as one undifferentiated section — which meant the exemptions the calibration relies on were dead, and `path-enumeration` was firing on Key files sections *because* they comply. A heuristic that fires on compliance is worse than one that misses, and it was invisible because each finding still read as plausible. Promotion criterion: an id becomes a warn only once its false-fire share over accumulated doctor runs is low enough that the warn is actionable, not noise.
