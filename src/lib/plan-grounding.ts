...
import { selectPlanFeatures } from "./context-pack.js";
...
export interface PlanGrounding {
  ...
  omissions: Array<{ input: string; reason: "unowned" | "unknown-feature" | "unreadable-doc" }>;
}
...
// Pure, deterministic projection of a plan's Scope and Map into the adversary's
...
export function buildPlanGrounding(input: PlanGroundingInput): PlanGrounding {
  ...
  const selection = selectPlanFeatures(registry, rows, input.scope ?? []);
  const routed = new Set(selection.selected);
  ...
  const unknownFeatures = [...new Set([...routed, ...dependency])].filter((slug) => !registry.features[slug]).sort();
  const omissions: PlanGrounding["omissions"] = [
    ...selection.unowned.map((input) => ({ input, reason: "unowned" as const })),
    ...unknownFeatures.map((input) => ({ input, reason: "unknown-feature" as const })),
  ];

  const features: FeatureGrounding[] = [];
  const project = (slug: string, relation: FeatureGrounding["relation"]): void => {
    const entry = registry.features[slug];
    if (!entry) return; // unknown routed slugs and dependencies are reported separately
  ...
  return { features, unknownFeatures, omissions };
}
...
export function gatherPlanGrounding(
  ...
  const slugs = new Set(selectPlanFeatures(registry, rows, scope).selected);
  ...
  return buildPlanGrounding({ rows, scope, registry, docContents });
}
